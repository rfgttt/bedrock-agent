...
**这不是一个 ChatGPT 壳。** 薄壳应用把提示词转发给模型再打印回复；这里每一次请求都走显式循环——
意图 → 能力路由 →（可选）审批门 → 执行 → 可验证结果——会产生副作用的文件工具被限在 workspace 内，
默认拒绝远程/局域网客户端，持久变更一律要求显式审批（验证、日志、审批三件套）。见 [SECURITY.md](SECURITY.md)。
...
来自 CLI 循环（`bedrock_agent.cli`）：敏感请求在落盘之前会停下来等人工决策（示意）：

```text
你> 把 workspace/ 里的发票文件按月整理归档
     → 能力：file_organizer（先生成计划，不移动任何文件；仅限 workspace/）
     → 状态：approval_required（展示待执行计划供审查）
你> approve
     → 计划执行；某步失败时，已完成的移动自动逆序撤销
       （已成功的计划也支持 undo 撤销）
```

只读工具无需门禁即可运行。写入与外部效果——包括应用启动、媒体控制、网络访问、
文件变更、持久记忆写入、技能安装与 MCP 调用——均需显式审批；危险工具默认拒绝。
...
更新可复现、改动可验证、失败可回滚。本仓库将这一发布流程与运行时实现一并保留，任何改动都可被检视和复现。
